...
class Scenario():
  ...
  def setup_nodes(self, session):
    ...
    """
    # This is where we can generate node using for loop
    for node in self.mace_nodes:
      ...
      ####  https://coreemu.github.io/core/python.html#interface-helper
      
      core_node = session.add_node(CoreNode, options=node.options)
      #  CoreEmu create_session  :  source code in github
      ...
      # This is all the log file in the tracer and we can personlize it.
  ...
  def start_etcd(self, session):
    ...
    for node in self.etcd_cluster:
      ...
      command = "/usr/bin/etcd --data-dir=" + self.etcd_cluster[node]['disk'] 
      command += " --name " + node
