...
class UTMServer():
  ...
  def compare_distances(self):
    ...
    for aircraft_id, data in to_write:
        ...
        self.write_to_etcd_complementary()        
  ...
  def write_to_etcd_complementary(self):
    """
    确保本地存储的所有数据也正确地存储在 ETCD 中。
    此函数从 ETCD 获取所有数据，与本地存储进行比较，
    并将任何本地存储中缺失的数据写入 ETCD。
    """
    try:
        # 从 ETCD 获取所有键值对，并提取所有的键
        etcd_entries = self.etcd.get_all()
        etcd_keys = set(metadata.key for value, metadata in etcd_entries)  # 假设 metadata 中包含 key 属性

        # 收集所有本地存储的键
        local_keys = {data.split(';')[1] for data in self.data_bank}

        # 找到 ETCD 中缺失的键
        missing_keys = local_keys - etcd_keys
        print(self.tag, missing_keys)

        # 用本地存储中缺失的数据更新 ETCD
        for data_str in self.data_bank:
            aircraft_id = data_str.split(';')[1]
            if aircraft_id in missing_keys:
                # 假设 data_str 已经是适合存储的格式，或者需要重新构造 data
                data = ";".join(data_str.split(';')[2:])
                self.write_to_etcd(aircraft_id, data)
                print(f"{self.tag} Complementarily saved {aircraft_id} to ETCD.")

    except Exception as e:
        print(f"{self.tag} Error during ETCD complementary update: {str(e)}")
